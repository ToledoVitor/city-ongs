...
4. ⚠️ **JSON builder + local schema validation** — Convênio done as the reference implementation; the other 4 ajuste types not started. New `audesp` app: `audesp/validators.py` validates a built payload against the downloaded JSON Schema files (`docs/audesp/`) using `jsonschema`, with a custom `multipleOf` override (see below). `audesp/builders/convenio.py` assembles all 26 blocks Convênio's schema requires from the Phase 0 models. Proven end-to-end against a real (sqlite, in-memory) ORM fixture — not just import-checked: `build_payload()` → `validate_payload()` returns zero errors. `audesp/models.AudespSubmission` tracks built payloads (append-only — one row per build/submit attempt, not a singleton, since AUDESP's own retificação flow means multiple attempts over time). Remaining: builders for Contrato de Gestão, Termo de Colaboração, Termo de Fomento, Termo de Parceria, Declaração Negativa — same pattern, different block subset per the §3 matrix.
