...
class AccountabilityCSVExporter:
    ...
    def __build_receipt_worksheet(self):
        ...
        for main_colum, sub1, sub2 in header_content:
            if col == 0: 
                receipt_worksheet.merge_range(0, 0, 1, 0, main_colum, header_breaking_line_format)
            elif sub1 == "":
    ...
    def __build_expense_worksheet(self):
        ...
        header_content = (
            ...
            [" COMPETÊNCIA ", "", "", ""],
            ["FONTE DE RECURSO", "Nome", "ID (não preencher)", ""],
            ["NATUREZA DA DESPESA (somente para despesa NÃO planejada)", "Nome", "ID (não preencher)", ""],
            ["FAVORECIDO/CONTRATADO", "Nome", "CPF/CNPJ", "ID (não preencher)"],
            ["ITEM DE AQUISIÇÃO (somente despesa planejada)", "Especifição", "ID (não preencher)", ""],
        ...
        for main_colum, sub1, sub2, sub3 in header_content:
            if col == 0: 
                expense_worksheet.merge_range(0, 0, 1, 0, main_colum, header_breaking_line_format)
            elif sub1 == "":
                # Adiciona texto e mesclas células 1 e 2 de suas respectivas colunas
    ...
    def __build_application_worksheet(self):
        ...
        for main_colum, sub1, sub2 in header_content:
            if col == 0: 
                application_worksheet.merge_range(0, 0, 1, 0, main_colum, header_breaking_line_format)
            elif sub1 == "":
                # Adiciona texto e mesclas células 1 e 2 de suas respectivas colunas
                application_worksheet.merge_range(0, col, 1, col, main_colum, header_format)
        ...
        for id in range(2, 1002):
            ...
            formula = f'=IFERROR(VLOOKUP(D{id+1},$CB.A$2:$CB.B$100,2),"")' # fórmula coluna E
            ...
            formula = f'=IFERROR(VLOOKUP(J{id+1},$FR.A$2:$FR.B$100,2),"")' # fórmula coluna K
