...
class PassOn11PDFExporter(CertificationTermPDFExporter):
    def __init__(self, contract, start_date, end_date, responsibles=None):
        ...
        self.labels = TermLabels(
            header_title="ANEXO RP-11 - TERMO DE CIÊNCIA E DE NOTIFICAÇÃO",
            header_subtitle="(REPASSES AO TERCEIRO SETOR - TERMO DE CONVÊNIO)",
            certainty_a_text=(
            ...
            personal_data_text=(
                "as informações pessoais dos responsáveis pelo órgão público convenente e "
                "entidade conveniada estão cadastradas no módulo eletrônico do 'Cadastro "
            ...
            grantor_authority_title="AUTORIDADE MÁXIMA DO ÓRGÃO PÚBLICO CONVENENTE:",
            grantor_orderer_title="ORDENADOR DE DESPESA DO ÓRGÃO PÚBLICO CONVENENTE:",
            beneficiary_authority_title="AUTORIDADE MÁXIMA DA ENTIDADE CONVENIADA:",
            conclusion_section_title="Responsáveis que assinaram o ajuste e/ou Parecer Conclusivo:",
            conclusion_signature_label="PELO ÓRGÃO PÚBLICO CONVENENTE:",
            account_section_title="Responsáveis que assinaram o ajuste e/ou prestação de contas:",
            account_signature_label="PELA ENTIDADE CONVENIADA:",
        )
    ...
    def _info_lines(self) -> list[str]:
        ...
        return [
            f"**ÓRGÃO PÚBLICO CONVENENTE:** {self.contract.organization.city_hall.name}",
            f"**ENTIDADE CONVENIADA:** {self.contract.hired_company} ({self.contract.area.name})",
            f"**TERMO DE CONVÊNIO N°(DE ORIGEM):** {self.contract.name}",
