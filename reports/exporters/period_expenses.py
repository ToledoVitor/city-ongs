...
@dataclass
class PeriodEpensesPDFExporter(CommonPDFExporter):
    """PDF exporter for period expenses reports."""
    ...
    def _draw_bank_transfers_table(self) -> None:
        ...
        body_data = [
            [
                "Entradas",
                "(+)",
                format_into_brazilian_currency(income),
            ],
            [
                "Saídas" if self.investing_account else "Saídas",
                "(-)",
                format_into_brazilian_currency(outgoing),
            ],
        ]
