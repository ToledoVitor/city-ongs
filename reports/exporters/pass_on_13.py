...
# Text constants
TITLE = "ANEXO RP-13 - TERMO DE CIÊNCIA E DE NOTIFICAÇÃO"
SUBTITLE = (
    "(REPASSES AO TERCEIRO SETOR - "
    "AUXÍLIOS/SUBVENÇÕES/CONTRIBUIÇÕES)"
)
SUBTITLE_NOTE = (
    "(utilização apenas para os repasses anteriores à edição da "
    "LF 13019/2014 atualizada)"
)
...
FOOTNOTES = [
    ...
    (
        "(*) - O Termo de Ciência e de Notificação deve identificar as pessoas "
        "físicas que tenham concorrido para a prática do ato jurídico, na "
        "condição de ordenador da despesa; de partes contratantes; de "
...
@dataclass
class PassOn13PDFExporter(CommonPDFExporter):
    """Exporter for Pass On 13 PDF report."""
    ...
    def _draw_header(self) -> None:
        ...
        self.set_font(font_size=6, bold=False)
        self.draw_cell(
            text=SUBTITLE_NOTE,
            width=190,
            align="C",
    ...
    def _draw_notification(self) -> None:
        ...
        for text in NOTIFICATION_TEXT[:2]:
            self.draw_cell(text=text, markdown=True)
            self.ln(4)
        self.set_font(font_size=8)
        for text in NOTIFICATION_TEXT[2:]:
            self.draw_multi_cell(text=text, width=190, markdown=True)
            self.ln(1)
        self.ln(10)
    ...
    def _draw_notificated(self) -> None:
        ...
        self.draw_cell(text=NOTIFICATED_TEXT[0])
        self.ln(4)
        self.set_font(font_size=8, bold=False)
        for text in NOTIFICATED_TEXT[1:]:
            self.draw_multi_cell(text=text, width=190, markdown=True)
            self.ln(1)
        self.ln(10)
    ...
    def _draw_public_authority(self) -> None:
        ...
        self.draw_cell(text="AUTORIDADE MÁXIMA DO ÓRGÃO PÚBLICO CONCESSOR:")
        self.ln(4)
        self.set_font(font_size=8, bold=False)
        self.draw_cell(text=f"Nome: {self.contract.organization.city_hall.mayor}")
        self.ln(4)
        self.draw_cell(text=f"Cargo: {self.contract.organization.city_hall.position}")
        self.ln(4)
        self.draw_cell(
    ...
    def _draw_expenditure_orderer(self) -> None:
        ...
        self.set_font(font_size=8, bold=True)
        self.draw_cell(text="ORDENADOR DE DESPESA DO ÓRGÃO PÚBLICO CONCESSOR:")
        self.ln(4)
        self.set_font(font_size=8, bold=False)
        self.draw_cell(text=f"Nome: {self.contract.contractor_manager.name}")
        self.ln(4)
        self.draw_cell(text=f"Cargo: {self.contract.contractor_manager.name}")
        self.ln(4)
        self.draw_cell(text=document_mask(str(self.contract.contractor_manager.cnpj)))
        self.ln(5)
    ...
    def _draw_beneficiary_authority(self) -> None:
        ...
        self.set_font(font_size=8, bold=True)
        self.draw_cell(text="AUTORIDADE MÁXIMA DO ENTIDADE BENEFICIÁRIA:")
        self.ln(4)
        self.set_font(font_size=8, bold=False)
        self.draw_cell(text=f"Nome: {self.contract.organization.owner}")
        self.ln(4)
        self.draw_cell(text=f"Cargo: {self.contract.organization.position}")
        self.ln(4)
        self.draw_cell(text=document_mask(str(self.contract.organization.document)))
        self.ln(10)
    ...
    def _draw_conclusion_signature_owner(self) -> None:
        ...
        self.draw_cell(
            text=(
                "Responsáveis que assinaram o ajuste e/ou "
                "Parecer Conclusivo:"
            )
        )
        self.ln(4)
        self.draw_cell(text="PELO ÓRGÃO PÚBLICO CONCESSOR:")
        self.ln(4)
        self.set_font(font_size=8, bold=False)
        self.draw_cell(text="Nome: Comitê")
        self.ln(4)
        self.draw_cell(text="Cargo: Comitê")
        self.ln(4)
        self.draw_cell(text=document_mask(str(self.contract.supervision_autority.cpf)))
        self.ln(10)
    ...
    def _draw_account_signature_owner(self) -> None:
        ...
        self.ln(4)
        self.draw_cell(text="PELA ENTIDADE BENEFICIÁRIA:")
        self.ln(4)
        self.set_font(font_size=8, bold=False)
        self.draw_cell(
            text=(
                f"Nome: "
                f"{self.contract.accountability_autority.get_full_name()}"
            )
        )
        self.ln(4)
        self.draw_cell(text=f"Cargo: {self.contract.accountability_autority.position}")
        self.ln(4)
        self.draw_cell(
    ...
    def _draw_other_responsable(self) -> None:
        ...
        self.set_font(font_size=8, bold=True)
        self.draw_cell(text="DEMAIS RESPONSÁVEIS (*):")
        self.ln(6)
        ...
        if responsibles:
            for responsible in responsibles:
                ...
                self.ln(6)
                self.draw_cell(text=f"Nome: {responsible['user'].get_full_name()}")
                self.ln(6)
                self.draw_cell(text=f"Cargo: {responsible['user'].position}")
                self.ln(6)
                self.draw_cell(
            ...
            self.set_font(font_size=8, bold=False)
            self.draw_cell(text="Tipo de ato sob sua responsabilidade:")
            self.ln(6)
    ...
    def _draw_footnote(self) -> None:
        ...
        for footnote in FOOTNOTES:
            self.draw_multi_cell(text=footnote, width=190, markdown=True)
            self.ln(1)
